...
export default function StudentDetailPage({ id }: Props) {
  ...
  const onCertSubmit = (values: z.infer<typeof certSchema>) => {
    createCertMutation.mutate({ data: { ...values, studentId: id } as any }, {
      onSuccess: () => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="mt-6">
              ...
              <DialogContent className="max-w-md" dir="rtl">
                ...
                <Form {...certForm}>
                  <form onSubmit={certForm.handleSubmit(onCertSubmit)} className="space-y-4">
                    <FormField control={certForm.control} name="title" render={({ field }) => (
                      <FormItem>
                        <FormLabel>عنوان الشهادة</FormLabel>
                        <FormControl><Input placeholder="مثال: شهادة إتمام جزء عم" {...field} /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                    <FormField control={certForm.control} name="issuedAt" render={({ field }) => (
                      <FormItem>
                    ...
                    <FormField control={certForm.control} name="description" render={({ field }) => (
                      <FormItem>
                    ...
                    <Button type="submit" className="w-full" disabled={createCertMutation.isPending}>
                      إصدار الشهادة
                    </Button>
                  </form>
                </Form>
          ...
          {certificates && certificates.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-10 text-muted-foreground bg-muted/20 rounded-xl border-2 border-dashed border-muted">
              <Award className="w-10 h-10 mb-2 text-muted-foreground/40" />
              <p className="text-sm">لم تُصدر شهادات لهذا الطالب بعد</p>
              <p className="text-xs mt-1">اضغط "إصدار شهادة" لمنح الطالب شهادة تكريم</p>
            </div>
          )}
        </div>
      </div>
